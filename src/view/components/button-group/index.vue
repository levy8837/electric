<template>
    <div class="operate">
        <span class='operate-btn'>
            <Button type="success" icon="refresh" shape="circle" class='add' @click='refreshHandler'>刷新</Button>
        </span>
        <span class='operate-btn' v-if='addStatus'>
            <Button type="primary" icon="plus-round" shape="circle" @click='addHandler'>新增</Button>
        </span>

        <span class='operate-btn' v-if='shipmentStatus'>
            <Button type="warning" icon="edit" shape="circle" @click='shipmentHandler'>
                出货
            </Button>
        </span>
        ...
        <span class='operate-btn' v-show='ignoredStatus'>
            <Button type="primary" icon="chatboxes" shape="circle" @click='ignoredHandler'>已忽略</Button>
        </span>
        <span class='operate-btn' v-show='permissions'>
            <Button type="warning" icon="permissions" shape="circle" @click='permissionsHandler'>授权</Button>
        </span>
        <span class='operate-btn' v-show='supplyStatus'>
            <Button type="warning" icon="edit" shape="circle" @click='supplyHandler'>补充订单</Button>
        </span>

        <span class='operate-btn' v-show='dynamicBtn.status'>
            <Button type="warning" icon="edit" shape="circle" @click='dynamicBtnHandler'>{{dynamicBtn.text}}</Button>
        </span>
        <span class='operate-btn' v-show='operate.status'>
            <Button type="warning" icon="edit" shape="circle" @click='handleOperateClick'>
                {{operate.text}}
            </Button>
        </span>
        ...
        <span class='operate-btn' v-show='deleteStatus'>
            <Button type="error" icon="trash-a" shape="circle" @click='deleteHandler(data)'>删除</Button>
        </span>
        <span class='operate-btn' v-show='finishStatus'>
            <Button type="warning" icon="edit" shape="circle" @click='tofinishOrder'>完成</Button>
        </span>
        <span class='operate-btn' v-show='confirmStatus'>
            <Button type="primary" icon="plus-round" shape="circle" class='confirm' @click='handleConfirmClick'>
                确认出货
            </Button>
        </span>
        <Dropdown trigger="click" @on-click='handmoreclick' v-show='moreOperations.length'>
            ...
            <DropdownMenu slot="list">
                <DropdownItem :name='item.name' v-for='(item, index) in moreOperations' :key="index">
                    {{item.name}}
                </DropdownItem>
            </DropdownMenu>
        </Dropdown>
        ...
        <Modal v-model="deletteModelStatus" width="310" :styles="{top: '250px'}">
            ...
            <div slot="footer">
                <Button type="info" size="large" @click="handleCancel">
                    取消
                </Button>
                <Button type="info" size="large" @click="confirmDeleteHandler">
                    确定
                </Button>
            </div>
        </Modal>
        <Modal v-model="operatModelStatus" width="310" :styles="{top: '250px'}">
            ...
            <div slot="footer">
                ...
                <Button type="info" size="large" @click="sureOperate">
                    确定
                </Button>
            </div>
        </Modal>
        <Modal v-model="sureModelStatus" width="310" :styles="{top: '250px'}">
            ...
            <div slot="footer">
                ...
                <Button type="info" size="large" @click="sureShipment">
                    确定
                </Button>
            </div>
        </Modal>
